	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','sendiosnotifications','sendanroidnotifications','flashsalenotificationcron'),
				'users'=>array('*'),
			),
			...
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','admin','index','view','sendPush','create','update','admin','delete','sendnotifications','sendanroidnotifications','flashsalenotificationcron'),
				'users'=>array('admin'),
			),
        ...
        public function actionCreate()
	{
		...
		if(isset($_POST['FlashSale'])){
			...
			// echo '<pre>';print_r($_POST);die;
                        // get last insert id of created flash sale
                        $insert_id = Yii::app()->db->getLastInsertID();
                        
                        /* select reccords from user table and bind 2 static colums fore insert 
                          Create an array for batch insert
                        */
                        
                        $sql = "select user_id,email, device_type,device_token,'".$insert_id."' AS flash_sale_id, 0 as send_flag , '".$_POST['FlashSale']['sale_start_from']."' as sale_start_from from users where device_token != ''";
                        $usermodel = Yii::app()->db->createCommand($sql)->queryAll();
                        
                        //only for 1.1.14
                        $insertbuilder = Yii::app()->db->schema->commandBuilder;
                        $insertcommand=$insertbuilder->createMultipleInsertCommand('user_flashsale_notification', $usermodel);
...
//                 
	...
	protected function performAjaxValidation($model)
	{
       ...
       /**
	* Cron Action when flash sale is created for sending notification.
        * 
	*/  
       public function actionFlashsalenotificationcron()
       {
            #### Send Notification to IOS users ####
            $wb=new Webservice();
 	    //For IOS users 
	    $wb->actionpushNotificationFlashSaleToIos("flash","");
	    // for anroid users
            $wb->actionpushNotificationFlashSaleToAndroid();
