...
class UsersController extends Controller
{
	...
	public function accessRules()
	{
			...
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','create','update','admin','delete','sendnotifications','ajaxsendNotificaton','Testinfo'),
...
public function actionAjaxsendNotificaton()
{
    ...
    {
	$message=array("msg"=>$_POST['messege'],"id"=>"-1","type"=>"notification");
	 ...
	 $response = Yii::app()->JsonWebservice->fetchData($post, $dataobj = 'db');
	  if($response['status']==1){
		...
		if($devicetoken != "")
		{
		    if ($device_type == 1)
		    { 
			if( $wb->actionpushNotificationToAndroid($message)) {
...
//                         
	...
	else
	{
	   ...
	   foreach($_POST['ids'] as $key=>$value)
	   {
		...
		$post["fields"] = 'device_token,device_type,user_id';
		$post["beforeWhere"] = '';
		$post["afterWhere"] = 'user_id = "'.$value.'" and device_token != ""';
		$post["r_p_p"] = '1';
		$post["start"] = 'all';
		$response = Yii::app()->JsonWebservice->fetchData($post, $dataobj = 'db');
		if($response['status']==1){
...
public function actionTestinfo()
{
	$url = 'https://android.googleapis.com/gcm/send';
	 ...
	 $key = 'AIzaSyBbLx8Uwy0xhnsoKzA4aiAmyRJDWgtyfrg' ;
        //echo $key ;
        $headers = array(
