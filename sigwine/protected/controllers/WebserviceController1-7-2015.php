...
public function actiongetReOrder() {
	...
	$netDiscount = 0;
        ...
        if (isset($fields['user_id']) && $fields['user_id'] != '') {
           ...
           $sqlallOrders = "Select count(*) As numofrows, order_total_id from res_order_total  where user_id = '". $fields['user_id'] ."' AND payment_status IN('Approved','Pending')";
		...
		if($netDiscount > 0)		
		{		
			/* Update Res_order_total for Discount */
			$data['order_discount'] = $netDiscount;
		        $where = " user_id='" . $fields['user_id'] . "' and order_total_id='" . $CountOrders[0]['order_total_id']. "'";
		        $response = $wb->updateData($data, '', '', $where, "res_order_total"); 
		}
  ...
  public function actionDeleteAutoPayOrder()
   {	   
	
	$wb = new Webservice();
        $fields = $_REQUEST;
	if (isset($fields['user_id']) && $fields['user_id'] != '' && isset($fields['order_total_id']) && $fields['order_total_id'] != '') {
		// Updte order is_auto_deleted flag

		$post['is_auto_deleted'] = '1';
                $where = " user_id='" . $fields['user_id'] . "' and order_total_id='" . $fields['order_total_id'] . "'";
                $response = $wb->updateData($post, '', '', $where, "res_order_total"); 
		
	}
	else {
	    $response = array('status' => '0', 'data' => array('message' => 'user_id or order_total_id is blank'));
        }
   }      
