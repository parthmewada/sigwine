...
<table class="tftable" border="1">
   ...
   ?>
    ...
    /* Check For Discount */
    $sqlallOrders = "Select count(*) As numofrows from res_order_total  where user_id = '". $userInfo['user_id'] ."' AND payment_status IN('Approved','Pending')";
    $CountOrders = $wb->getAllData($sqlallOrders);
     // Get Discount Amt For Order
    if(isset($CountOrders) && $CountOrders[0]['numofrows'] == '0')
       {
            ...
            <?php if ($productInfo) { $i=1; $netAmt=0; ?>
                <table width="100%" border="1" class="tftable">
                    ...
                    <?php foreach ($productInfo as $pk => $pv) { ?>
                        <?php if ($pv["type"] == 1) { 
			    if($pv['subscription_duration'] != 'auto')	
			    {	
                          	$sqlprice = "select price from subscription_plan_price where plan_id= ".$pv['subscription_plan_id']." AND duration=".$pv['subscription_duration'];
                            	$priceInfo = $wb->getAllData($sqlprice);
			    	$subprice = $priceInfo[0]['price']; 
			   }
			   else
			    	$subprice = $pv['order_total'];  				
			   ?>
                            <tr>
                                <td><b><?php echo $i++; ?></b></td>
                                <td><b><?php echo $pv["name"]; ?></b><br/>
				<?php   if($pv['subscription_duration'] != 'auto') {?>
                                    <i>(Subscribed for <?php echo $pv["subscription_duration"]; ?>Month)</i>
				<?php } else { ?>
				      <i>Auto</i>
				<?php } ?>	
				</td>
                                ...
                                <td><?php echo $subprice; $netAmt+= $subprice ;?></td>
                            </tr>
                            <tr>
                                <td>&nbsp;</td>
                    ...
                    <?php  } ?>
                     <?php if($discountPerc) { ?>
                        <tr>
                            <td colspan="4" style="text-align: right;"><b>Total(&yen;)</b>:</td><td><?php echo $discountAmt = ($netAmt * $discountPerc)/100 ;?></td>
                        </tr>   
                     <?php } ?>       
                    <tr>
