...
class Discount extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('discount_perc, status', 'required'),
			array('discount_perc', 'numerical'),
			array('status', 'length', 'max'=>8),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('discount_id, discount_perc, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('discount_perc',$this->discount_perc);
